...
csl: https://raw.githubusercontent.com/liao961120/local_depend/master/style_sheets/apa-no-doi-no-issue.csl
link-citations: yes
---
...
接下來將透過幾段文字和幾張圖片[^opendata]，描述臺灣語言的現況[^reliability]，並會聚焦在**跨年齡層**以及**跨世代**的語言流失現象。


[^opendata]: 對於不滿足於網頁所提供之資訊者，可至網站[開放資料](./open_data.html)頁面取得更多相關資訊。
...
下圖顯示子女(問卷填寫者)與父、母語言能力的差異，同樣以語族[^langgroup2]為基礎：
...
[^langgroup2]: 這裡一樣使用類似附註 4 對語族的定義。不同的是，此將父、母分開：在計算子女與父親的語言能力差異時，僅以"父親是否會講該語言"作為篩選；計算子女與母親語言能力差異時，僅以母親篩選。
...
語言流失中的一個明顯跡象可見於世代之間溝通語言的差異。你是否有過這兩種經驗之一：

1. 平常使用方言講話，但在和晚輩說話時，卻不由自主地改成國語。

2. 長輩和自己說話時，會自動由常用的方言改成國語。

造成這種現象的可能原因不只一種：可能單純因為晚輩方言程度不佳，不易進行溝通；也有可能來自**對不同語言的態度**以及**對語言學習的錯誤認識**[^multilang] --- 即便對於尚在學習語言關鍵時期(7 歲前)的孩童，許多長輩仍使用國語和孩童說話[^codeswitch]。


[^codeswitch]: 這在臺北地區十分明顯：祖父母常使用國語和年幼的孫兒講話。
